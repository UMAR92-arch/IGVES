          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(INITIAL_ASSETS).map(([name, data]) => (
              <div 
                ...
                className={`group relative p-8 rounded-2xl border ${getBorderColor(data.change)} ${getBgColor(data.change)} hover:scale-[1.02] hover:border-emerald-500/50 transition-all cursor-pointer overflow-hidden shadow-2xl min-h-[240px]`}
              >
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Aktiv turi: {name === 'BTC' ? 'Kripto' : name === 'GOLD' ? 'Tovar' : name === 'FOREX' ? 'Valyuta' : 'Iqtisodiy'}</span>
                  </div>
                ...
                <div className="mb-8">
                  <div className="text-4xl font-mono font-light tracking-tighter">
                    ...
                    <span className="text-xs text-neutral-600 ml-2">{name === 'FOREX' ? 'USD' : name === 'BTC' || name === 'GOLD' ? '$' : '%'}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-tighter">
                  ...
                  <div className="flex gap-1 h-8 items-end">
                    {[1,4,2,6,3,8,4,9].map((h, i) => (
                      <div key={i} className="w-1 bg-emerald-500/40 rounded-t" style={{ height: `${h * 10}%` }}></div>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="p-8 rounded-2xl border border-neutral-800 bg-neutral-900/40 flex flex-col justify-center items-center text-center group hover:bg-neutral-800/60 transition-all min-h-[240px]">
               ...
               <h3 className="text-lg font-bold mb-2">AI Deep Insights</h3>
    ...
    <div className="flex h-screen w-screen bg-[#050505] overflow-hidden text-neutral-200">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 overflow-x-auto no-scrollbar max-w-[60vw]">
              {Object.entries(INITIAL_ASSETS).map(([name, data]) => (
                <button 
                  ...
                  className={`flex flex-col items-start min-w-[90px] px-3 py-1 rounded transition-all ${selectedAsset === name ? 'bg-emerald-500/10 border border-emerald-500/30' : 'opacity-40 hover:opacity-100'}`}
                >
                  <div className="text-[9px] font-bold text-neutral-500 uppercase">{name}</div>
                  <div className="flex gap-2 items-baseline">
                    <span className="text-xs font-mono">{data.price.toFixed(name === 'FOREX' ? 4 : 2)}</span>
        ...
        <div className="flex-1 bg-black relative trading-view-grid overflow-hidden">
           {chartData.length > 0 ? (
             <TradingChart data={chartData} assetName={selectedAsset} inspectorMode={inspectorMode} />
           ) : (
             <div className="h-full w-full flex items-center justify-center text-neutral-600 font-mono text-xs">YUKLANMOQDA...</div>
           )}
        </div>
      </div>
        ...
        <div className="w-80 border-l border-neutral-800 bg-[#0a0a0a] flex flex-col animate-in slide-in-from-right overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <button 
              ...
              className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 disabled:bg-neutral-900 text-black font-black rounded-xl transition-all text-xs uppercase tracking-widest"
            >
              {isAnalyzing ? "TAHLIL QILINMOQDA..." : "TAHLILNI BOSHLASH"}
            </button>

            {aiAnalysis && (
              <div className="p-5 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                <div className="text-[10px] text-emerald-500 font-bold mb-3 uppercase">AI SIGNAL (UZBEK)</div>
